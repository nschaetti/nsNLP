...
class JoinConverter(Converter):
    ...
    def __init__(self, conv1, conv2):
        ...
        """
        super(JoinConverter, self).__init__()

        # Check that there is not One-Hot or Letter converter
        if type(conv1) is OneHotConverter.OneHotConverter or type(conv1) is LetterConverter.LetterConverter or \
            type(conv2) is OneHotConverter.OneHotConverter or type(conv2) is LetterConverter.LetterConverter:
            raise Exception(u"Cannot join with a converter of type OneHot or Letter")
        # end if
