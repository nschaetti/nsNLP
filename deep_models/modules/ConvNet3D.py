...
# Convolution module with 3D kernels
class ConvNet3D(nn.Module):
    ...
    def __init__(self, n_classes=2, channels=(1, 10, 2, 20, 2, 4800, 50), kernel_size=5, stride=1):
        ...
        self.conv1 = nn.Conv3d(channels[0], channels[1], kernel_size=kernel_size, stride=stride)

        # 2D convolution layer, 10 input channels, 20 output channels (filters), kernel size 5
        self.conv2 = nn.Conv3d(channels[1], channels[3], kernel_size=kernel_size)
    ...
    def forward(self, x):
        ...
        :return:
        """
        # ReLU << Max pooling 3D with kernel size 2 << Convolution layer 1 << x
        x = self.conv1(x)
        x = F.max_pool3d(x, self._max_pool1_size)
        x = F.relu(x)

        # ReLU << Max pooling 2D with kernel size 2 << Dropout 2D << Convolution layer 2 << x
        x = F.relu(F.max_pool3d(self.conv2_drop(self.conv2(x)), self._max_pool2_size))

        # Put all 320 features into 1D line << x
        x = x.view(-1, ConvNet3D.num_flat_features(x))
